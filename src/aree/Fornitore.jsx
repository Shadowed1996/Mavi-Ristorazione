...
import {
  AGGREGATO, ALLERGENI, CATEGORIE, splitPiatto, CICLICO, COLORI, FLUSSI_ORDINE, GIORNI, GIRI, IMPOSTAZIONI_INIZIALI, INGREDIENTI_DIETE, MARCATORI, PIATTI, catalogoPerCategoria, sostituisce,
} from "../data.js";
import {
  Accesso, DiscoColore, Documenti, Icone, Illustrazione, Ingredienti, Intestazione, Messaggi, Marchio,
  ...
  return (
    <>
      ...
      <div className="tela">
        ...
        <div className="numeri">
          <div className="numero"><div className="n-lab">Ordini ricevuti</div><div className="n-val">{FLUSSI_ORDINE.filter((x) => x.stato !== "attesa").length}</div><div className="n-nota">su {FLUSSI_ORDINE.length} attesi</div></div>
...
function ImpostazioniServizio() {
  ...
  const [ciclico, setCiclico] = React.useState(() => CICLICO.map((c) => ({ ...c })));
  const [editCiclico, setEditCiclico] = React.useState(null);
  const [editCiclicoVal, setEditCiclicoVal] = React.useState({ nome: "", nota: "", stato: "" });
  const strutture = [
  ...
  return (
    <>
      ...
      <div className="tela">
        ...
        <div className="pannello">
          <div className="pannello-testa">
            <h2>Menu ciclico a rotazione, quattro settimane</h2>
            <span className="conta-piatti">rotazione autunnale</span>
          </div>
          <div className="ciclico">
            {ciclico.map((w, i) => (
              <div key={w.n} className={"ciclico-voce " + w.stato}>
                <span className="ciclico-num">{w.n}</span>
                <div style={{ flex: 1 }}>
                  {editCiclico === i ? (
                    <div className="ciclico-edit">
                      <input type="text" value={editCiclicoVal.nome} onChange={(e) => setEditCiclicoVal((v) => ({ ...v, nome: e.target.value }))} placeholder="Nome" />
                      <input type="text" value={editCiclicoVal.nota} onChange={(e) => setEditCiclicoVal((v) => ({ ...v, nota: e.target.value }))} placeholder="Nota" />
                      <select value={editCiclicoVal.stato} onChange={(e) => setEditCiclicoVal((v) => ({ ...v, stato: e.target.value }))}>
                        <option value="attiva">attiva</option>
                        <option value="programmata">programmata</option>
                        <option value="in revisione">in revisione</option>
                      </select>
                      <button className="btn piccolo" onClick={() => {
                        setCiclico((p) => p.map((c, j) => j === i ? { ...c, ...editCiclicoVal } : c));
                        setEditCiclico(null);
                        st.avvisa("Settimana " + w.n + " aggiornata");
                      }}>OK</button>
                      <button className="btn linea piccolo" onClick={() => setEditCiclico(null)}>×</button>
                    </div>
                  ) : (
                    <>
                      <b>{w.nome}</b>
                ...
                <span className="pastiglia p-neu">{w.stato}</span>
                {editCiclico !== i && (
                  <button className="btn linea piccolo" style={{ marginLeft: 8 }} onClick={() => {
                    setEditCiclico(i);
                    setEditCiclicoVal({ nome: w.nome, nota: w.nota, stato: w.stato });
                  }}>Modifica</button>
                )}
              </div>
            ))}
          ...
          <div className="pannello-piede">
            Alla fine della quarta settimana si torna alla prima. Le rotazioni si cambiano quattro
            volte l'anno per seguire la stagionalità.
          </div>
        </div>
      </div>
    </>
...
/* ==================== cronoprogramma di sviluppo ==================== */
function Cronoprogramma() {
  const FASI = [
    { n: 1, nome: "Analisi e capitolato", quando: "Settembre 2026", durata: "3 settimane", stima: "compresa nel prezzo di partenza",
      dipende: "conferma dei quattro tipi di committente da realizzare",
      cosa: "Interviste con MAVI e con una struttura per tipo, capitolato tecnico, schemi dei flussi, definizione delle credenziali di responsabilità sui dati sanitari." },
    { n: 2, nome: "Impianto tecnico e portale azienda", quando: "Ottobre 2026", durata: "4 settimane", stima: "€ 3.500 lordi (già preventivati)",
      dipende: "capitolato firmato",
      cosa: "Server, database, autenticazione, portale dipendente e portale referente aziendale, catalogo piatti, distinta di produzione, resoconti, fatture PDF." },
    { n: 3, nome: "Portale RSA", quando: "Novembre 2026", durata: "3 settimane", stima: "€ 2.400 lordi, indicativi",
      dipende: "capitolato dati sanitari con il DPO della struttura",
      cosa: "Griglia dei nuclei, anagrafica ospiti, scheda paziente, diete e consistenze, etichette monoporzione." },
    { n: 4, nome: "Portale comunità e scuola", quando: "Dicembre 2026", durata: "3 settimane", stima: "€ 2.200 lordi, indicativi",
      dipende: "menu vidimato ATS per le scuole",
      cosa: "Modelli a unità e a presenze, tabella grammature per fascia d'età, calendario chiusure scolastiche." },
    { n: 5, nome: "Cucina centrale MAVI", quando: "Gennaio 2027", durata: "3 settimane", stima: "€ 2.000 lordi, indicativi",
      dipende: "portali cliente conclusi",
      cosa: "Distinta multi struttura, ordini in arrivo, giri di consegna, etichette, impostazioni per committente, menu ciclico." },
    { n: 6, nome: "Test e formazione", quando: "Febbraio 2027", durata: "2 settimane", stima: "€ 800 lordi, indicativi",
      dipende: "primo cliente disposto a fare da pilota",
      cosa: "Test con dati veri, formazione degli operatori, correzioni finali, manuale utente." },
    { n: 7, nome: "Messa in produzione", quando: "Marzo 2027", durata: "1 settimana", stima: "canone di hosting a parte",
      dipende: "consenso al trattamento e nomine responsabili firmati",
      cosa: "Migrazione, monitoraggio, assistenza prime due settimane in affiancamento." },
  ];
  const totale = "€ 10.900 lordi, indicativi";
  return (
    <>
      <Intestazione
        occhiello="Sviluppo del prodotto"
        titolo="Cronoprogramma"
        sotto="Fasi previste, tempi di massima e stime economiche per riferimento"
      ...
      <div className="tela">
        <div className="avviso info" style={{ alignItems: "flex-start" }}>
          <Icone.attenzione size={18} />
          <span>
            I tempi partono dal via libera dell'azienda e presuppongono capitolato firmato entro
            l'inizio della fase successiva. Le stime economiche sono ordini di grandezza per
            avere un riferimento in riunione, il preventivo definitivo si costruisce a
            capitolato letto. La fase 2 è quella già confermata a € 3.500 lordi.
          </span>
        </div>

        <div className="fasi">
          {FASI.map((f) => (
            <article className="fase" key={f.n}>
              <div className="fase-testa">
                <span className="fase-numero">{f.n}</span>
                <div>
                  <span className="occhiello">{f.quando} · {f.durata}</span>
                  <h3>{f.nome}</h3>
                </div>
                <span className="fase-stima">{f.stima}</span>
              </div>
              <div className="fase-corpo">
                <p>{f.cosa}</p>
                <p className="fase-dep"><b>Dipende da:</b> {f.dipende}</p>
              </div>
            </article>
          ))}
        </div>
        ...
        <div className="pannello">
          <div className="pannello-testa">
            <h2>Riepilogo</h2>
            <span className="conta-piatti">tutte le fasi</span>
          </div>
          <div className="scorri">
            <table className="dati">
              <tbody>
                <tr><td><b>Durata complessiva</b></td><td>circa sei mesi calendariali</td></tr>
                <tr><td><b>Impegno effettivo</b></td><td>circa dodici settimane di lavoro attivo</td></tr>
                <tr><td><b>Stima economica totale</b></td><td className="cifra"><b>{totale}</b></td></tr>
                <tr><td><b>Modalità</b></td><td>acconto 50% alla firma di ciascuna fase, saldo alla consegna</td></tr>
                <tr><td><b>Non incluso</b></td><td>canone di hosting mensile, trasmissione fatture allo SDI, provider di pagamento online</td></tr>
              </tbody>
            </table>
          </div>
          <div className="pannello-piede">
            Le stime non sostituiscono un preventivo firmato. Servono a rispondere in modo
            trasparente alla domanda quanto costa senza vincolare né MAVI né il fornitore.
          </div>
        </div>
      </div>
    </>
...
/* ==================== roadmap di sviluppo e stime ==================== */
function Roadmap() {
  const st = usaStato();
  const FASI = [
    {
      n: 1, nome: "Fondamenta e portale aziendale",
      quando: "Mesi 1 e 2, ottobre e novembre",
      cose: [
        "Impianto tecnico, autenticazione, gestione utenti e committenti",
        "Portale dipendente con menu, scheda piatto e vassoio",
        "Portale referente aziendale con anagrafica, resoconti e fatture PDF",
        "Distinta di produzione per la cucina, esportazione Excel e PDF",
      ],
      stima: "da 6.000 a 8.000 €", stato: "prima consegna", tono: "prima",
    },
    {
      n: 2, nome: "RSA, comunità e diete complesse",
      quando: "Mesi 3 e 4, dicembre e gennaio",
      cose: [
        "Portale operatore di nucleo con griglia quantità, diete e consistenze",
        "Anagrafica ospiti, scheda ospite, collegamento con l'ordine",
        "Consenso al trattamento e messa in regola dei dati sanitari (GDPR art. 9)",
        "Portale comunità nella variante casa per casa",
      ],
      stima: "da 5.000 a 7.000 €", stato: "seconda consegna", tono: "seconda",
    },
    {
      n: 3, nome: "Scuola paritaria e vidimazione ATS",
      quando: "Mese 5, febbraio",
      cose: [
        "Portale insegnante con rilevazione presenze e diete certificate",
        "Grammature per fascia d'età, menu vidimato",
        "Portale segreteria, resoconti, fatture con tariffa comunale",
      ],
      stima: "da 2.500 a 3.500 €", stato: "terza consegna", tono: "terza",
    },
    {
      n: 4, nome: "Cucina centrale, catalogo, menu ciclico",
      quando: "Mese 6, marzo",
      cose: [
        "Compositore menu con rotazione a quattro settimane e stagionalità",
        "Catalogo piatti completo, caricamento foto, gestione allergeni",
        "Etichette pasto stampabili nominative e collettive",
        "Giri di consegna con orari, autisti e tappe",
      ],
      stima: "da 3.500 a 4.500 €", stato: "quarta consegna", tono: "quarta",
    },
    {
      n: 5, nome: "Manutenzione, formazione, avviamento",
      quando: "Mese 7, aprile in poi",
      cose: [
        "Formazione degli utenti chiave, un pomeriggio per profilo",
        "Migrazione dati anagrafici iniziali",
        "Assistenza per i primi 60 giorni di esercizio",
        "Piano di manutenzione evolutiva annuale",
      ],
      stima: "canone da concordare, indicativamente da 150 a 300 € al mese",
      stato: "in esercizio", tono: "manutenzione",
    },
  ];
  const somma = "da circa 17.000 a 23.000 €, escluse manutenzione, hosting e domini";
  return (
    <>
      <Intestazione
        occhiello="Piano di lavoro"
        titolo="Roadmap di sviluppo"
        sotto="Cinque fasi, sei mesi al totale, con stime di massima per fase"
        azioni={<button className="btn linea piccolo" onClick={() => window.print()}>
          <Icone.stampa size={16} /> Stampa
        </button>}
      />
      <div className="tela">
        <div className="avviso info" style={{ alignItems: "flex-start" }}>
          <Icone.attenzione size={18} />
          <span>
            Numeri e mesi sono <b>stime di massima</b>, non un preventivo definitivo. Il preventivo
            si scrive dopo il capitolato, verificati volumi, integrazioni con i sistemi esistenti,
            richieste di personalizzazione e responsabilità sui dati sanitari.
          </span>
        </div>

        <div className="roadmap">
          {FASI.map((f) => (
            <article className={"fase fase-" + f.tono} key={f.n}>
              <div className="fase-testa">
                <span className="fase-num">Fase {f.n}</span>
                <span className="fase-stato">{f.stato}</span>
              </div>
              <h3>{f.nome}</h3>
              <div className="fase-quando">{f.quando}</div>
              <ul className="fase-cose">
                {f.cose.map((c) => <li key={c}>{c}</li>)}
              </ul>
              <div className="fase-stima">
                <span className="so-lab">Stima di massima</span>
                <b>{f.stima}</b>
              </div>
            </article>
          ))}
        ...
        <div className="pannello">
          <div className="pannello-testa">
            <h2>Totale piattaforma completa, tutti e quattro i mondi</h2>
            <span className="conta-piatti">senza IVA</span>
          </div>
          <div style={{ padding: 22 }}>
            <p style={{ margin: 0, fontFamily: "var(--serif)", fontSize: 22, fontWeight: 500 }}>{somma}</p>
            <p style={{ margin: "12px 0 0", fontSize: 13, color: "var(--muto)", lineHeight: 1.55 }}>
              Il singolo caso aziendale, senza gli altri tre mondi, resta al prezzo di ingresso concordato
              a parte. Modalità di pagamento suggerite: acconto del cinquanta per cento alla firma, saldo
              alla consegna di ciascuna fase, oppure suddivisione a stati di avanzamento con verifica
              congiunta.
            </p>
          </div>
        </div>
