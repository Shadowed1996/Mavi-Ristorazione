...
import {
  CATEGORIE, COLORI, COMMITTENTI, CONSISTENZE, DIETE_TERAPEUTICHE, FASCE_SCOLASTICHE,
  MODELLI, OSPITI_RSA, PIATTI, menuDelGiorno, sostituisce,
} from "../data.js";
...
function MenuOggi({ giorno = 2, titolo = "Menu di oggi", nota }) {
  const st = usaStato();
  return (
    <div className="pannello menu-oggi">
      <div className="pannello-testa">
        <h2>{titolo}</h2>
        {nota && <span className="conta-piatti">{nota}</span>}
      </div>
      <div className="menu-oggi-corpo">
        {CATEGORIE.map((c) => {
          const lista = menuDelGiorno(st.menu, giorno, c.id);
          if (!lista.length) return null;
          return (
            <section className="menu-oggi-cat" key={c.id}>
              <h4>{c.nome}</h4>
              <div className="menu-oggi-lista">
                {lista.map((id) => (
                  <div className="menu-oggi-piatto" key={id}>
                    <span className="disco-mini"><Illustrazione id={id} /></span>
                    <div>
                      <b>{PIATTI[id].n}</b>
                      <span>
                        <DiscoColore colore={PIATTI[id].col} size={9} /> {COLORI[PIATTI[id].col].nome} · {PIATTI[id].kcal} kcal
                        {PIATTI[id].a.length ? " · allergeni " + PIATTI[id].a.join(", ") : ""}
                        {PIATTI[id].so ? " · " + sostituisce(id) : ""}
                      </span>
                    </div>
                  </div>
                ))}
      ...
      <div className="pannello-piede">
        Il menu lo governa la cucina MAVI. Se cambia, la modifica arriva subito qui.
      </div>
    </div>
  );
}
...
export function OrdiniUnita({ tipo, nudo, utente }) {
  const st = usaStato();
        ...
        azioni={
          <>
            ...
            <button className="btn piccolo" onClick={() => {
              const totale = righe.reduce((a, r) => a + Object.keys(r).filter(k => k !== "unita").reduce((s, k) => s + r[k], 0), 0);
              st.trasmettiOrdine({
                ...
                unita: (utente && utente.mansione) || "Nucleo", pasti: totale,
...
/* ==================== presenze, scuole ==================== */
export function Presenze({ nudo, utente }) {
  const st = usaStato();
  const c = committenteAttivo(st.committente);
      ...
      <Intestazione
        ...
        azioni={
          <>
            ...
            <button className="btn piccolo" onClick={() => {
              const presenti = st.presenze.reduce((a, r) => a + r.presenti, 0);
              const diete = st.presenze.reduce((a, r) => a + r.diete, 0);
              st.trasmettiOrdine({
                struttura: c.id, strutturaNome: c.nome,
...
export function ModelliServizio() {
  ...
  const [stati, setStati] = React.useState(
    () => Object.fromEntries(COMMITTENTI.map((c) => [c.id, "attivo"]))
  );
  ...
  return (
    <>
      <Intestazione
        ...
        titolo="Anagrafica committenti"
        sotto="Le strutture servite dalla cucina MAVI. Da qui si attivano, sospendono o configurano."
        azioni={
          <button className="btn piccolo" onClick={() => st.avvisa("Nuovo committente: funzione dimostrativa. In produzione si compila il form completo")}>
      ...
      <div className="tela">
        <div className="numeri">
          <div className="numero"><div className="n-lab">Committenti</div><div className="n-val">{COMMITTENTI.length}</div><div className="n-nota">strutture censite</div></div>
          <div className="numero"><div className="n-lab">Attivi</div><div className="n-val">{Object.values(stati).filter((s) => s === "attivo").length}</div><div className="n-nota">ricevono il servizio</div></div>
          <div className="numero"><div className="n-lab">Pasti al giorno</div><div className="n-val">{COMMITTENTI.reduce((s, c) => s + c.pasti, 0)}</div><div className="n-nota">stima complessiva</div></div>
        </div>

        <div className="comm-griglia">
          {COMMITTENTI.map((c) => {
            const attivo = stati[c.id] === "attivo";
            return (
              <div className="comm-card" key={c.id} style={{ opacity: attivo ? 1 : 0.55 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <span className={"pastiglia " + (attivo ? "p-ok" : "p-att")} style={{ marginBottom: 8, display: "inline-block" }}>
                      {attivo ? "attivo" : "sospeso"}
                    </span>
                    <h3>{c.nome}</h3>
                  </div>
                  <span className="pastiglia p-neu">{c.tipo}</span>
                </div>
                <div className="comm-info">
                  <div>Modello: <b>{MODELLI[c.modello].nome}</b></div>
                  <div>Referente: <b>{c.nota || "da assegnare"}</b></div>
                  <div>Unità: <b>{c.unita.length} {c.etichettaUnita.toLowerCase()}</b></div>
                  <div>Pasti stimati: <b>{c.pasti}/giorno</b></div>
                  <div>Chi ordina: {MODELLI[c.modello].chi}</div>
                  <div>Chi paga: {MODELLI[c.modello].paga}</div>
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: "auto" }}>
                  <button className="btn linea piccolo" style={{ flex: 1 }}
                    onClick={() => { st.setCommittente(c.id); st.avvisa("Committente " + c.nome + " selezionato come attivo"); }}>
                    Seleziona
                  </button>
                  <button className={"btn piccolo" + (attivo ? " linea" : "")}
                    style={attivo ? { color: "#d9534f" } : { background: "#5cb85c" }}
                    onClick={() => {
                      setStati((p) => ({ ...p, [c.id]: attivo ? "sospeso" : "attivo" }));
                      st.avvisa(c.nome + (attivo ? " sospeso" : " riattivato"));
                    }}>
